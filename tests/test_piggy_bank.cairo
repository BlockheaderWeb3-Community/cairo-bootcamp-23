...
use blockheader_cairo_bootcamp::piggy_bank::{
    piggyBankTraitDispatcher, piggyBankTraitDispatcherTrait, IERC20Dispatcher,
    IERC20DispatcherTrait,
};
use snforge_std::{
    declare, ContractClassTrait, start_prank, stop_prank, start_warp, stop_warp, env::var,
    ContractClass, get_class_hash, CheatTarget
};
...
fn deploy_contract_forked(
    ...
    let deployed_contract_address: ContractAddress =
        0x042f0284511fb30a93defacad0c7d2a8968bfe0700fb54785337753e3b12720a
        .try_into()
        .unwrap();
    // Precalculate the address to obtain the contract address before the constructor call (deploy) itself
    let contract_address = ContractClass { class_hash: get_class_hash(deployed_contract_address) }
        .precalculate_address(@calldata);
    start_prank(CheatTarget::One(contract_address), owner.try_into().unwrap());
    let deployedContract = ContractClass { class_hash: get_class_hash(deployed_contract_address) }
        .deploy(@calldata)
        .unwrap();
...
fn get_important_addresses() -> (ContractAddress, ContractAddress, ContractAddress,) {
    let caller: ContractAddress = 0x032e0bbab381cdc21c523699add33f9df9c80444ae174f81413f3122f4ed7b1f
        .try_into()
        .unwrap();
    let EthToken: ContractAddress =
