...
use blockheader_cairo_bootcamp::piggy_bank::piggyBank::targetOption;


#[starknet::interface]
trait IPiggyBankFactory<TContractState> {
    fn createPiggyBank(
        ref self: TContractState, savingsTarget: targetOption, targetDetails: u128
    ) -> ContractAddress;
...
mod piggyFactory {
    ...
    use blockheader_cairo_bootcamp::ownership_component::IOwnable;
    use core::serde::Serde;
    use starknet::{ContractAddress, ClassHash, get_caller_address, Zeroable};
    use starknet::syscalls::deploy_syscall;
    use super::targetOption;
    use blockheader_cairo_bootcamp::ownership_component::ownable_component;
    ...
    impl piggyFactoryImpl of super::IPiggyBankFactory<ContractState> {
        fn createPiggyBank(
            ref self: ContractState, savingsTarget: targetOption, targetDetails: u128
        ) -> ContractAddress {
            ...
            self
                .emit(
                    HashUpdated {
                        ...
                        oldHash: self.piggyBankHash.read(),
                        newHash: newClasHash
                    }
                );
        }
